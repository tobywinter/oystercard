...
describe Oystercard do
  ...
  describe '#top_up' do
    ...
    it "raises error 'Exceeds maximum limit' message" do
      expect {card.top_up(71)}.to raise_error("Exceeds maximum limit: Balance must not exceed £#{Oystercard::MAX_LIMIT}")
    end
  ...
  describe "#touch_in" do
    ...
    it "will raise error 'Insufficient funds' if below minimum balance" do
      expect {empty_card.touch_in(entry_station)}.to raise_error("Insufficient funds: Minimum balance - £#{Oystercard::MINIMUM_BALANCE}")
    end
